...
class Module(nn.Module):
    ...
    configs: Sequence[Config]  # 专家配置列表,每个专家一个配置,主要是gemma和action
    ...
    def setup(self):
        ...
        block_cls = nn.remat(
            Block,
            prevent_cse=False,
            static_argnums=(5,),  # 5=deterministic (确定性参数为静态参数)
            policy=jax.checkpoint_policies.nothing_saveable,  # 检查点策略，不保留任何中间激活值
        )
        # 使用nn.scan构建Transformer层,实现层之间的共享变量轴
        self.layers = nn.scan(
            block_cls,
            variable_axes={"params": 0},  # 参数在层之间共享，通俗来说：如果你有 length=18 层，那么 nn.scan 不会创建 18 组完全独立的参数。相反，它会创建一个大的参数张量，这个张量的第一个轴有 18 个元素，每个元素对应一层。这样，你就可以在训练过程中拥有 18 组独立的层参数。如果没有这个设置，Flax 会默认在层之间共享参数（即所有层使用同一组权重）。
            split_rngs={"params": True, "dropout": True},  # 分割随机数生成器，"params": True：表示在每次循环时，为参数初始化的 RNG 生成一个新的子密钥，"dropout": True：表示在每次循环时，为Dropout操作的 RNG 生成一个新的子密钥
